    ...
    def _prepare_inputs(self, inputs: dict[str, Union[torch.Tensor, Any]]) -> dict[str, Union[torch.Tensor, Any]]:
        ...
        for i, (reward_func, reward_processing_class) in enumerate(
            ...
            if isinstance(reward_func, nn.Module):  # Module instead of PretrainedModel for compat with compiled models
                ...
                # TODO now compatable with verl reward function make universal format
                # reward_kwargs = {key: [example[key] for example in inputs] for key in keys}
                ground_truths = [x['reward_model']['ground_truth'] for x in inputs]
                output_reward_func = [reward_func(solution_str=completion[0]['content'], ground_truth=ground_truth) for completion, ground_truth in zip(completions, ground_truths)]
